...
if st.session_state.user:
    ...
    if st.button("Сгенерировать", use_container_width=True):
        ...
        if credits < needed:
            ...
            # Генерация изображения (только если запрошено и текст успешен)
            image = None
            image_success = False
            if generate_image_flag and text_success:
                style_map = {"Реалистичный": "realistic", "Мультяшный": "cartoon", "Аниме": "anime", "Акварель": "watercolor", "3D": "3D", "Киберпанк": "cyberpunk"}
                detail_map = {"Быстрая": "", "Средняя": "high quality", "Высокая": "detailed", "Максимальная": "ultra detailed"}
                prompt = f"{topic}, {style_map[img_style]}, {detail_map[img_detail]}"
                if extra_details:
            ...
            # Человезация текста (только если запрошено и текст успешен)
            artifacts = []
            humanized_post = post
            ...
            # Фактчекинг (только если запрошено и текст успешен)
            ...
            if final_post and text_success:
                # Списание кредитов ТОЛЬКО за успешно выполненные операции
                credits_to_deduct = 1  # текст
                if image_success:
                    credits_to_deduct += 2
                ...
                # Показываем информацию о человезации
                ...
                # Показываем результат фактчекинга
                if factcheck_result:
...
# Принудительное открытие боковой панели при загрузке
st.markdown("""
<style>
